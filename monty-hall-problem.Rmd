...
There is a brain teaser that I couldn't to understand intuitively. So here is a demo that shows how it works practically. More details [https://en.wikipedia.org/wiki/Monty_Hall_problem](https://en.wikipedia.org/wiki/Monty_Hall_problem)

What's given:
1. There are 3 doors, there is the car behind one of them.
2. You choose one door.
3. Then one empty door is opened.
4. You're asked to choose the door again or stay if previous one.
...
```{r take_3, cache=TRUE, fig.width=4, fig.height=4}
...
```{r open_door, cache=TRUE, fig.width=4, fig.height=4}
...
```{r scene_1, cache=TRUE}
guessedWithScenario1 <- table(df$guessed)
guessedWithScenario1
```

We wins in __`r format(100 * guessedWithScenario1['TRUE']/(guessedWithScenario1['FALSE'] + guessedWithScenario1['TRUE']), nsmall=2, digits=2)`%__ of cases.

```{r scene_1_pie, cache=TRUE, fig.width=4, fig.height=4}
...
```{r scene_2, cache=TRUE}
df$choiceTwo <- 6 - df$choiceOne - df$doorOpened
guessedWithScenario2 <- table(df$choiceTwo == df$car)
guessedWithScenario2
```

__Wow! It really works!__ We wins in __`r format(100 * guessedWithScenario2['TRUE']/(guessedWithScenario2['FALSE'] + guessedWithScenario2['TRUE']), nsmall=2, digits=2)`%__ of cases.


```{r scene_2_pie, cache=TRUE, fig.width=4, fig.height=4}
...
We wins in __`r format(100 * guessedWithScenario3['TRUE']/(guessedWithScenario3['FALSE'] + guessedWithScenario3['TRUE']), nsmall=2, digits=2)`%__ of cases.

```{r scene_3_pie, cache=TRUE, fig.width=4, fig.height=4}
...
__Change your opinion every time.__ | __66%__
...
Source code: [https://github.com/vladimir-kazan/monty-hall-simulation](https://github.com/vladimir-kazan/monty-hall-simulation)
